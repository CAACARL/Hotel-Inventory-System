...
class TransactionExportController extends Controller
{
    ...
    public function exportReports(Request $request)
    {
        ...
        $callback = function() use ($dateFrom, $dateTo) {
            ...
            // COMPLETE BATCHES SECTION
            fputcsv($file, [$this->createWideColumn('📦 COMPLETE BATCH INVENTORY', 120)]);
            fputcsv($file, [$this->createWideColumn('📊 All Batches with Expiry Tracking', 100)]);
            fputcsv($file, [$this->createWideColumn('📊 Total Batches: ' . number_format(Batch::count()), 90)]);
            fputcsv($file, [$this->createWideColumn('', 220)]);

            fputcsv($file, [
                $this->createWideColumn('🔖 Batch Number', 50),
                $this->createWideColumn('📦 Lot Number', 40),
                $this->createWideColumn('🏷️ Item Name', 60),
                $this->createWideColumn('📂 Category', 45),
                $this->createWideColumn('🔢 Quantity', 30),
                $this->createWideColumn('📏 Unit', 25),
                $this->createWideColumn('💰 Unit Cost', 30),
                $this->createWideColumn('🏢 Supplier', 45),
                $this->createWideColumn('📍 Location', 40),
                $this->createWideColumn('📅 Expiry Date', 35),
                $this->createWideColumn('🔄 Status', 30),
                $this->createWideColumn('📅 Created', 30)
            ]);

            $allBatches = Batch::with(['item.category'])->orderBy('created_at', 'desc')->get();

            foreach ($allBatches as $batch) {
                // Skip batches with missing item or category
                if (!$batch->item || !$batch->item->category) {
                    continue;
                }

                $expiryInfo = 'No expiry';
                if ($batch->expiry_date) {
                    $expiryInfo = $batch->expiry_date->format('Y-m-d');
                    if ($batch->isExpired()) {
                        $expiryInfo .= ' (EXPIRED)';
                ...
                fputcsv($file, [
                    $this->createWideColumn($batch->batch_number, 50),
                    $this->createWideColumn($batch->lot_number ?: 'N/A', 40),
                    $this->createWideColumn($batch->item->name, 60),
                    $this->createWideColumn($batch->item->category->name, 45),
                    $this->createWideColumn(number_format($batch->quantity), 30),
                    $this->createWideColumn($batch->item->unit, 25),
                    $this->createWideColumn($batch->unit_cost ? 'PHP ' . number_format($batch->unit_cost, 2) : 'N/A', 30),
                    $this->createWideColumn($batch->supplier ?: 'N/A', 45),
                    $this->createWideColumn($batch->location ?: 'N/A', 40),
                    $this->createWideColumn($expiryInfo, 35),
                    $this->createWideColumn(ucfirst($batch->status), 30),
                    $this->createWideColumn($batch->created_at->format('Y-m-d'), 30)
                ]);
            }
    ...
    /**
     * Export batches with beautiful design and extra wide columns
     */
    public function exportBatches(Request $request)
    {
        $filename = 'Icon_Venue_Suites_Batches_' . date('Y-m-d_H-i-s') . '.csv';
        ...
        $callback = function() {
            ...
            fputcsv($file, [$this->createWideColumn('🏢 ICON VENUE & SUITES', 120)]);
            fputcsv($file, [$this->createWideColumn('📦 BATCH REPORT - PROFESSIONAL EDITION', 150)]);
            fputcsv($file, [$this->createWideColumn('📅 Generated: ' . date('F j, Y \a\t g:i A'), 100)]);
            fputcsv($file, [$this->createWideColumn('📊 Total Batches: ' . number_format(Batch::count()), 80)]);
            fputcsv($file, [$this->createWideColumn('', 180)]);
            fputcsv($file, [$this->createWideColumn(str_repeat('═', 180), 180)]);
            fputcsv($file, [$this->createWideColumn('', 180)]);
            ...
            fputcsv($file, [
                ...
                $this->createWideColumn('🏢 Supplier', 45),
                $this->createWideColumn('📍 Location', 40),
                $this->createWideColumn('📅 Expiry Date', 30),
                $this->createWideColumn('🔄 Status', 30)
            ]);

            $batches = Batch::with(['item.category'])->orderBy('created_at', 'desc')->get();
            ...
            foreach ($batches as $batch) {
                ...
                fputcsv($file, [
                    ...
                    $this->createWideColumn($batch->supplier ?: 'N/A', 45),
                    $this->createWideColumn($batch->location ?: 'N/A', 40),
                    $this->createWideColumn($expiryInfo, 30),
                    $this->createWideColumn(ucfirst($batch->status), 30)
                ]);
            }
            ...
            fputcsv($file, [$this->createWideColumn('', 180)]);
            fputcsv($file, [$this->createWideColumn(str_repeat('═', 180), 180)]);
            fputcsv($file, [$this->createWideColumn('🏁 END OF BATCH REPORT', 100)]);
